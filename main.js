...
const deepEqual = (a, b) => (a !== undefined && b !== undefined &&
    a !== null && b !== null ?
    (typeof a === "number" ? a === b :
        (typeof a === "string" ? a === b :
            (typeof a === "object" ?
                (Object.keys(a).length !== Object.keys(b).length ?
                    false :
                    Object.keys(a).every((keyA) =>
                        deepEqual(a[keyA], b[keyA])
                    )
                ) :
                a === b
            )
...
const expect = (title, expected, actual) => (!deepEqual(expected, actual) ?
...
const from = (S) => assign({
    to: (N) => range(S, N),
    downTo: (N) => range(N, S).map((e, i) => i).map(e => S - e)
});

const repeat = (N, o) => range(1, N).map(() => o);
const zeros = (N) => repeat(N, 0);

const leftPadZeros = (targetLength, arr) => [
    ...zeros(targetLength - arr.length),
    ...arr
];
const rightPadZeros = (targetLength, arr) => [
    ...arr,
    ...zeros(targetLength - arr.length)
];
...
const use = (v, fn) => fn(v);
const leftTrim = (arr) => use(arr.findIndex((e) => e !== 0),
...
block("leftPadZeros", test, () => {
    ...
    expect("leftPadZeros [0, 1, 3, 1, 0, 0]", [0, 0, 1, 3, 1, 0, 0],
        leftPadZeros(7, [0, 1, 3, 1, 0, 0]));
...
block("rightPadZeros", test, () => {
    ...
    expect("rightPadZeros [0, 1, 3, 1, 0, 0]", [0, 1, 3, 1, 0, 0, 0],
        rightPadZeros(7, [0, 1, 3, 1, 0, 0]));
...
const L1 = function(front = [], back = [], negative = false, chunkSize = 10) {
    if (typeof front === "string") {
        ...
        front = str.substr(0, deliPos).split("")
            .map(e => Number(e));
        back = str.substr(deliPos + 1, str.length).split("")
            .map(e => Number(e));
    }
    ...
    const add = (other_) => {
        ...
        let result = from(other.length - 1).downTo(0)
            .map(i => {
    ...
    const compare = (other_, takeAbs = false) => {
        ...
        return differentIdx < 0 ? 0 :
            (!negative ? other[differentIdx] > me[differentIdx] ? -1 : 1 :
                other[differentIdx] < me[differentIdx] ? -1 : 1);
    };
    ...
    const sub = (other_) => {
        ...
        let carry = 0;
        let result = from(a.length - 1).downTo(0)
            .map(i => {
    ...
    const multiplySingleArray = (me, multiplier) => {
        let carry = 0;
        const num = from(me.length - 1).downTo(0)
            .map(idx => me[idx])
            .map(digit => {
    ...
    const mul = (other_) => {
        ...
        const result_ =
            Object.keys(shiftsByMultipliers)
            .map(multiplier => {
        ...
        return use(
            result_.shift(-2 * postcomma),
            shifted => negative !== other_.negative ? shifted.invert() : shifted
        );
    };

    const isZero = () => front.every(e => e === 0) && back.every(e => e === 0);

    const mostSignificentDigit = () => isZero() ?
        0 :
        use(
    ...
    const shift = (width) => {
        ...
        if (width > 0) {
            ...
            const newBack = newBackDigits < 1 ? [] :
                back.slice(back.length - newBackDigits, back.length);
            const newFront = [
                ...front,
                ...back.slice(0, back.length - newBackDigits),
                ...zeros(-newBackDigits)
            ];
...
if (test) {
    ...
    expect("Multiplication 123456789*123456789 = 15241578750190521",
        "15241578750190521", L("123456789").mul(L("123456789")).toString());

    expect("Multiplication 123456789*123456789 = 15241578750190521",
        "152415787501905.21", L("12345678.9").mul(L("12345678.9")).toString());
    ...
    expect("To string with four digits", "0.1234",
        L([0], [1, 2, 3, 4]).toString());
    ...
    expect("1 Adding two front-only", [3, 5, 7], L([1, 2, 3])
        .add(L([2, 3, 4])).front());
    expect("2 Adding two front-only", [], L([1, 2, 3])
        .add(L([2, 3, 5])).back());
    expect("2 Adding two front-only", [3, 5, 8], L([1, 2, 3])
        .add(L([2, 3, 5])).front());

    expect("Adding 0.1 and 0.9", "1", L([], [1]).add(L([], [9])).toString());

    expect("Adding 0.1 and 0.2", "0.3", L([], [1]).add(L([], [2])).toString());
    expect("Adding 0.09 and 0.02", "0.11",
        L([], [0, 9]).add(L([], [0, 2])).toString());
    expect("Adding 0.09 and 0.2", "0.29",
        L([], [0, 9]).add(L([], [2])).toString());


    expect("Adding 0.9 and 0.2", "1.1", L([], [9]).add(L([], [2])).toString());

    expect("Adding 987654 + 987654 = 1975308", "1975308",
        L([9, 8, 7, 6, 5, 4]).add(L([9, 8, 7, 6, 5, 4])).toString())

    expect("Adding 9876540000000000 + 9876540000000000 = 19753080000000000",
        "19753080000000000",
        L([9, 8, 7, 6, 5, 4, ...zeros(10)])
        .add(L([9, 8, 7, 6, 5, 4, ...zeros(10)])).toString());

    expect("Adding 12345.6789 + 12345.6789 = 24691.3578", "24691,3578",
        L([1, 2, 3, 4, 5], [6, 7, 8, 9]).add(L([1, 2, 3, 4, 5], [6, 7, 8, 9]))
        .toString(","));

    expect("Adding 123 + 1 = 124", "124",
        L([1, 2, 3], []).add(L([1], [])).toString(","));

    expect("Adding twice 123456789123456789123456789123456789123456789123456789123456789123456789123456789123456789123456789123456789123456789123456789123456789",
        ...
        L([...
            "123456789123456789123456789123456789123456789123456789123456789123456789123456789123456789123456789123456789123456789123456789123456789".split("").map(e => Number(e))
        ]).add(L(
            [...
                "123456789123456789123456789123456789123456789123456789123456789123456789123456789123456789123456789123456789123456789123456789123456789".split("").map(e => Number(e))
            ]
...
const strL = str => use(str.replace(",", ".").indexOf("."), deliPos =>
    deliPos < 0 ? strL(str + ".") :
    L(
        str.substr(0, deliPos).split("").map(e => Number(e)),
        str.substr(deliPos + 1, str.length).split("").map(e => Number(e)),
    ));
